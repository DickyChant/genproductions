...
C       b_sf_003 links partons 1 and 6 
      ELSEIF ((M.EQ.1 .AND. N.EQ.6).OR.(M.EQ.6 .AND. N.EQ.1)) THEN
        CALL SB_SF_003(P_BORN,WGT)

C       b_sf_004 links partons 2 and 5 
      ELSEIF ((M.EQ.2 .AND. N.EQ.5).OR.(M.EQ.5 .AND. N.EQ.2)) THEN
        CALL SB_SF_004(P_BORN,WGT)

C       b_sf_005 links partons 2 and 6 
      ELSEIF ((M.EQ.2 .AND. N.EQ.6).OR.(M.EQ.6 .AND. N.EQ.2)) THEN
        CALL SB_SF_005(P_BORN,WGT)

C       b_sf_006 links partons 5 and 6 
      ELSEIF ((M.EQ.5 .AND. N.EQ.6).OR.(M.EQ.6 .AND. N.EQ.5)) THEN
