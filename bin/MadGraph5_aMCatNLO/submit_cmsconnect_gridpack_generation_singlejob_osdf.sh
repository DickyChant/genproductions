...
cat<<-EOF
	...
	transfer_output_files = ${card_name}_${scram_arch}_${cmssw_version}_tarball.tar.xz
...
scram_arch="${6:-slc7_amd64_gcc10}"
cmssw_version="${7:-CMSSW_12_4_8}"
...
tar -zcf "$input_files" "$card_dir" "$patches_directory" "runcmsgrid_LO.sh" "runcmsgrid_NLO.sh" "cleangridmore.sh" "/usr/bin/unzip" "Utilities" "MetaData"
